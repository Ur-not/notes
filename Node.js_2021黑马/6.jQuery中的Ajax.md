...
防抖策略（debounce）是当事件被触发后，延迟n秒后再执行回调，如果在这n秒内事件又被触发，则重新计时。这样当事件被频繁触发时，事件只会执行y
...
![](https://gitee.com/ur-not/picture/raw/master/24.3.2.png)
...
请求行、请求头、空行、请求体(post)
![](https://gitee.com/ur-not/picture/raw/master/25.2.png)

### 25.2.1 请求行

组成部分：请求方式、URL、HTTP协议版本
...
请求头部常见的请求头字段
![](https://gitee.com/ur-not/picture/raw/master/25.2.2.png)

### 25.2.3 空行

最后一个请求头字段的后面是一个空行，通知服务区请求头部至此结束
请求消息的空行:用来分隔请求头部和请求体

### 25.2.4 请求体(post)

请求体中存放POST方式提交到服务器的数据
...
状态行、响应头部、空行、响应体
![](https://gitee.com/ur-not/picture/raw/master/26.2.png)

### 26.2.1 状态行

组成部分：HTTP协议版本、状态码、状态码的描述文本

### 26.2.2 响应头部

响应头部原来描述服务器的基本信息。响应头部由多行键值对组成，每行键和值之间用英文的冒号分隔

常见的响应头字段
![](https://gitee.com/ur-not/picture/raw/master/26.2.2.png)

### 26.2.3 空行

在最后一个响应头部字段结束之后，会紧跟一个空行，用来通知客户端响应头部到此结束
响应消息中的空行，用来分隔响应头部与响应体

### 26.2.4 响应体

响应体中存放服务器响应给客户端的资源内容
...
![](https://gitee.com/ur-not/picture/raw/master/27.1.png)
...
## 28.2 HTTP响应状态码的组成及分类

HTTP状态码由三个十进制数字组成，第一个十进制数字定义了状态码的类型，后两个数字用来对状态码进行细分。HTTP状态码共分为5种类型：
![](https://gitee.com/ur-not/picture/raw/master/28.2.png)
...
### 28.3.2   3×× 重定向相关的响应状态码

3＊范围的状态码，表示表示服务器要求客户端重定向，需要客户端进一步的操作以完成资源的请求。常见的＊＊类型的状态码如下：
![](https://gitee.com/ur-not/picture/raw/master/28.3.2.png)

### 28.3.3   4×× 客户端错误相关的响应状态码

![](https://gitee.com/ur-not/picture/raw/master/28.3.3.png)

### 28.3.4   5×× 服务端错误相关的响应状态码

![](https://gitee.com/ur-not/picture/raw/master/28.3.4.png)
